...
function GasStations() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        'https://qc.hpcfuelrite.com/api/v2/admin/bulkImport/gasStations',
        formData,
        {
          headers: {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-r from-blue-400 to-purple-600 relative">
      {/* Background Overlay */}
      <div className="absolute inset-0 bg-black opacity-30"></div>

      {/* Main Content */}
      <div className="relative z-10 bg-white p-8 rounded-3xl shadow-2xl max-w-md w-full">
        <h2 className="text-3xl font-bold text-center mb-6 text-gray-800">Upload Gas Stations</h2>
